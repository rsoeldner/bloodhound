...
newtype TweetMapping = TweetMapping (Maybe ParentChildJoin) deriving (Eq, Show)
instance ToJSON TweetMapping where
  toJSON (TweetMapping pcj) =  object ["properties" .= object
   ...
   <> maybe [] (\x -> [joinField x .= toJSON x]) pcj
    )
                                        ]

pcLink :: ParentChildJoin
pcLink = ParentChildJoin { joinField   = "myJoinField"
...
resetIndex :: (Maybe ParentChildJoin) -> BH IO ()
resetIndex pcj = do
  _ <- deleteExampleIndex
  _ <- createExampleIndex
  _ <- putMapping testIndex (TweetMapping pcj)
  return ()
