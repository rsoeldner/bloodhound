...
module Database.V6.Bloodhound.Types
       ( defaultCache
...
import           Database.V6.Bloodhound.Types.Class
import           Database.V6.Bloodhound.Internal.Analysis
import           Database.V6.Bloodhound.Internal.Aggregation
import           Database.V6.Bloodhound.Internal.Client
import           Database.V6.Bloodhound.Internal.Highlight
import           Database.V6.Bloodhound.Internal.Newtypes
import           Database.V6.Bloodhound.Internal.Query
import           Database.V6.Bloodhound.Internal.Sort
import           Database.V6.Bloodhound.Internal.Suggest
...
linkedDocParseError :: String
linkedDocParseError = "LinkedDoc record toJSON returned non-object value"

data ParentChildJoin = ParentChildJoin
  { joinField   :: Text
  , parentField :: Text
  , childField  :: Text
  } deriving (Eq, Generic, Show)

instance ToJSON ParentChildJoin where
  toJSON ParentChildJoin{..} =
    object [ "type"      .= ("join" :: Text)
           , "relations" .= object [parentField .= childField]
           ]

data LinkedDoc a = Parent               (a,ParentChildJoin)
                 | Child DocumentParent (a,ParentChildJoin)

                 deriving (Eq, Show)
instance (ToJSON a) => ToJSON (LinkedDoc a) where
  toJSON (Parent (d, ParentChildJoin j p _)) = case toJSON d of
    Object m -> Object $ m <> HM.singleton j (object ["name" .= p])
    _        -> error linkedDocParseError
  toJSON (Child (DocumentParent (DocId pId)) (d,ParentChildJoin j _ c)) = case toJSON d of
    Object m -> Object $ m <> HM.singleton j (object ["name" .= c, "parent" .= pId])
    _        -> error linkedDocParseError
