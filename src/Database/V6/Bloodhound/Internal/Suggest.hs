...
module Database.V6.Bloodhound.Internal.Suggest where
...
import           Database.V6.Bloodhound.Internal.Query (TemplateQuery(..), tqParams)
...
data PhraseSuggesterCollate = PhraseSuggesterCollate
  { phraseSuggesterCollateTemplateQuery :: TemplateQuery
...
instance ToJSON PhraseSuggesterCollate where
  toJSON PhraseSuggesterCollate{..} =
    object [ "query" .= object
             [ "source" .= (tqSource phraseSuggesterCollateTemplateQuery)
             ]
           , "params" .= (tqParams phraseSuggesterCollateTemplateQuery)
...
instance FromJSON PhraseSuggesterCollate where
  parseJSON (Object o) = do
    ...
    return $ PhraseSuggesterCollate
             (TemplateQuery source' params') prune'
