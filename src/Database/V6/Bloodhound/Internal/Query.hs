...
module Database.V6.Bloodhound.Internal.Query
  ( module X
  , module Database.V6.Bloodhound.Internal.Query
  , TemplateSource (..)
...
data Query =
  ...
  | ParentIdQuery               (Text, DocumentParent)
  deriving (Eq, Show)
...
instance ToJSON Query where
  ...
  toJSON (IdsQuery docIds) =
    ...
    where conjoined = [ "type"   .= defaultMapping
  ...
  toJSON (ParentIdQuery (p, DocumentParent (DocId pId))) =
    object [ "parent_id" .= object ["type" .= p, "id" .= pId] ]
...
instance FromJSON Query where
  parseJSON v = withObject "Query" parse v
    where parse o = termQuery `taggedWith` "term"
                ...
                <|> parentIdQuery `taggedWith` "parent_id"
          ...
          parentIdQuery o = ParentIdQuery <$> o .: "parent_id"
...
instance ToJSON TemplateSource where
  toJSON TemplateSource{..} = object ( "query" .= query : rest )

instance FromJSON TemplateSource where
  parseJSON = withObject "Source" parse
    where
      parse o = do
        TemplateSource <$> o .: "query"
                       <*> pure (filter ((/=) "query" . fst) $ HM.toList o)

data TemplateQuery =
  TemplateQuery { tqSource :: TemplateSource -- Query
                , tqParams :: TemplateQueryKeyValuePairs
                }
  deriving (Eq, Show)

instance ToJSON TemplateQuery where
  toJSON TemplateQuery{..} = object [ "source" .= tqSource
                                    , "params" .= tqParams
                                    ]

instance FromJSON TemplateQuery where
  parseJSON = withObject "TemplateQuery" parse
    where parse o = TemplateQuery
                    <$> o .: "source"
                    <*> o .: "params"
