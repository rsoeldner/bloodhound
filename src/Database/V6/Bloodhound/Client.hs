...
module Database.V6.Bloodhound.Client
...
import           Database.V6.Bloodhound.Types
import           Database.V6.Bloodhound.Internal.Newtypes


-- $setup
-- >>> :set -XOverloadedStrings
-- >>> :set -XDeriveGeneric
-- >>> import Database.V6.Bloodhound
-- >>> import Network.HTTP.Client
...
putMapping (IndexName indexName) mapping =
  bindM2 put url (return body)
  where url = joinPath [indexName, "_mapping", defaultMapping]
...
indexDocument (IndexName indexName) cfg document (DocId docId) =
  bindM2 put url (return body)
  where url = addQuery params <$> joinPath [indexName, defaultMapping, docId]
        parentParams = case idsParent cfg of
          Nothing                         -> []
          Just (DocumentParent (DocId p)) -> [ ("routing", Just p) ]
...
updateDocument (IndexName indexName) cfg patch (DocId docId) =
  ...
  where url = addQuery (versionCtlParams cfg) <$>
              joinPath [indexName, defaultMapping, docId, "_update"]
...
deleteDocument (IndexName indexName) (DocId docId) =
  delete =<< joinPath [indexName, defaultMapping, docId]
...
encodeBulkOperation (BulkIndexEncodingAuto (IndexName indexName) encoding) =
...
getDocument (IndexName indexName) (DocId docId) =
  get =<< joinPath [indexName, defaultMapping, docId]

-- | 'documentExists' enables you to check if a document exists. Returns 'Bool'
--   in IO
--
-- >>> exists <- runBH' $ documentExists testIndex (DocId "1")
documentExists :: MonadBH m => IndexName -> DocId -> m Bool
documentExists (IndexName indexName) (DocId docId) = do
  (_, exists) <- existentialQuery =<< url
  return exists
  where url = joinPath [indexName, defaultMapping, docId]
...
-- | 'searchByType', given a 'Search' and 'IndexName', will perform that
--   search against an index on an Elasticsearch server.
--
-- >>> let query = TermQuery (Term "user" "bitemyapp") Nothing
-- >>> let search = mkSearch (Just query) Nothing
-- >>> reply <- runBH' $ searchByType testIndex search
searchByType :: MonadBH m => IndexName -> Search -> m Reply
searchByType (IndexName indexName) = bindM2 dispatchSearch url . return
  where url = joinPath [indexName, defaultMapping, "_search"]

-- | 'searchByTemplate', given a 'TemplateQuery', will perform that
--   search against an index on an Elasticsearch server via template.
--
-- >>> let templateSource = TemplateSource {query = QueryMatchQuery $
...
-- >>>                        ]
-- >>>     templateQuery = TemplateQuery templateSource templateParams
-- >>>  reply <- runBH' $ searchByTemplate templateQuery
searchByTemplate :: MonadBH m => TemplateQuery -> m Reply
searchByTemplate queryTemplate =
  bindM2 post url (return body)
  where url = joinPath ["_search", "template"]
        body = Just $ encode queryTemplate
...
getInitialScroll (IndexName indexName) search' = do
    let url = addQuery params <$> joinPath [indexName, defaultMapping, "_search"]
